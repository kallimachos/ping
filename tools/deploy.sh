...
if [ "$TRAVIS_REPO_SLUG" == "kallimachos/ping" ] && \
    ...
    git clone --quiet \
        --branch=gh-pages \
        https://$GH_TOKEN@github.com/kallimachos/ping gh-pages > /dev/null
...
auto-pushed to gh-pages"
    ...
    if test `tput -T $TERM colors` -lt 256; then
        echo "Docs published to http://kallimachos.github.io/ping"
    else
        tput -T $TERM setaf 2
        echo "Docs published to http://kallimachos.github.io/ping"
        tput -T $TERM sgr0
